...
- **顶栏**（同一行）：
  ...
  - 右侧：页码（点击左右翻页）、重新扫描、全屏/退出、设置
- **应用网格**：高清图标 + 名称居中；一页放不下时左右滑动切换，松手自动吸附整页；支持方向键/PageUp/Down/Home/End 翻页；**全屏时图标、文字、间距按屏幕自适应放大铺满**
- **设置面板**（顶栏齿轮）：
  - 主题：明亮 / 深黑 / 跟随系统
  ...
  - 操作：重新扫描；**开机自动启动**（SMAppService 登录项，可在系统设置中管理）
...
- **四指/五指捏合**：捏合（手指间距收缩约 20%）时——窗口未在前台则**打开并全屏**，已在前台则收起。
  ...
  > 手指数按事件触点统计，仅四指/五指触发。
  > 监听带自愈机制：系统休眠唤醒、事件监听被系统禁用时自动恢复（另有 30 秒健康检查兜底）。
